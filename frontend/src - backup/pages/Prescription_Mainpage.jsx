...
const ProfessionalPrescriptionInterpreter = () => {
    ...
    const processPrescriptionOCR = useCallback(async () => {
        ...
        try {
            ...
            if (state.enhancedPreviewUrl) {
                ...
                // Use 'file' as key - works with both ML service and backend microservice
    ...
    // Priority 1: Use structured medications from the ML pipeline
        ...
        if (nerEntities.medications && nerEntities.medications.length > 0) {
            return nerEntities.medications.map((med, idx) => {
                // NER entities can be objects {text, score} or plain strings
                const medName = typeof med === 'object' ? (med.text || med.name || '') : String(med);
                const getDosage = (arr, i) => {
                    if (!arr || !arr[i]) return '';
                    return typeof arr[i] === 'object' ? (arr[i].text || '') : String(arr[i]);
                };
                return {
                    name: medName,
                    dosage: getDosage(nerEntities.dosages, idx),
                    frequency: getDosage(nerEntities.frequencies, idx),
                    duration: getDosage(nerEntities.durations, idx),
                    route: getDosage(nerEntities.routes, idx),
                    form: getDosage(nerEntities.forms, idx),
                    instructions: '',
                    confidence: typeof med === 'object' ? Math.round((med.score || 0.75) * 100) : 75,
                    source: 'ner'
                };
            });
        }
    ...
    const analyzeImage = async (imageFile) => {
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                ...
                if (canvas.width === 0 || canvas.height === 0) {
                    resolve({ brightness: 50, histogram: [], isLowLight: true, resolution: '0x0', aspectRatio: 1 });
                    return;
                }
...
<body>
    ...
    ${state.parsedData.warnings.length > 0 ? `
        <h2>Warnings</h2>
        ${state.parsedData.warnings.map(w => `<div class="warning">${w}</div>`).join('')}
    ` : ''}
    
    <div style="margin-top: 50px; padding-top: 20px; border-top: 1px solid #ddd; color: #666; font-size: 12px;">
        <p>Generated by Prescription Interpreter</p>
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} sm={6}>
                        <Box sx={{ 
                            ...
                            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1, textAlign: 'center' }}>
                                {state.imageSettings.brightness < 80 && 'Image may be too dark for OCR'}
                                ...
                                {state.imageSettings.brightness > 150 && 'Image may be overexposed'}
                            </Typography>
                        </Box>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6}>
                        <Box sx={{ 
                            ...
                            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1, textAlign: 'center' }}>
                                {state.imageSettings.contrast < 70 && 'Low contrast - text may be unclear'}
                                ...
                                {state.imageSettings.contrast > 160 && 'Very high contrast - details may be lost'}
                            </Typography>
                        </Box>
                    </Grid>
                    ...
                    <aside className="space-y-4">
                        ...
                        <div className="relative overflow-hidden backdrop-blur-xl bg-gradient-to-br from-blue-50/90 via-indigo-50/80 to-violet-50/90 border border-blue-200/50 rounded-2xl shadow-xl shadow-blue-500/10 p-5">
                            ...
                            <div className="relative space-y-2">
                                {[
                                    { label: 'Check Interactions', path: '/interactions' },
                                    { label: 'Food Interactions', path: '/food-drug' },
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                            <div className="space-y-4">
                                ...
                                <p className="text-sm text-gray-400 leading-relaxed">
                                    AI-powered platform for analyzing prescription images, extracting medications, dosages, and clinical instructions.
                                </p>
                                <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="p-4 rounded-xl bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20">
                                    <p className="text-xs text-gray-300 leading-relaxed">
                                        <span className="text-amber-400 font-semibold">For informational purposes only.</span> Always verify with original prescription and consult healthcare professionals.
                                    </p>
                                </div>
                            </div>
                        </div>
