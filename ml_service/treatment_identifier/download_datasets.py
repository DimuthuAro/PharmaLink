"""
Treatment Identifier – Step 1: Dataset Download
=================================================
Downloads all datasets required for treatment/condition identification
from prescription medications.

Datasets:
  1. Medical Information Dataset (MID.xlsx) – Drug names, therapeutic classes, action classes
  2. Drug-Food Interactions Dataset (JSON) – Drug names with usage context
  3. DrugBank Drug-Drug Interactions (CSV) – Interaction descriptions with treatment context

Usage:
  python -m treatment_identifier.download_datasets            # interactive mode
  python -m treatment_identifier.download_datasets --all      # download all, no prompts
  python -m treatment_identifier.download_datasets --verify   # check status only
"""
...
DATASETS = [
    ...
    },
    {
        ...
        "description": "Drug names with food interaction context and usage details",
        "auto": True,
    },
    {
        "id": 3,
        ...
        "description": "~191k drug-drug interaction pairs – contains treatment context in descriptions",
        "auto": True,
    },
]
...
def banner():
    print(f"""
{C.CYAN}╔══════════════════════════════════════════════════════════════╗
║{C.RESET}{C.BOLD}{C.WHITE}   TREATMENT IDENTIFIER – Dataset Download                   {C.RESET}{C.CYAN}║
╚══════════════════════════════════════════════════════════════╝{C.RESET}
{C.DIM}  Component : Treatment Identifier
...
# ── Main ─────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Treatment Identifier – Dataset Downloader")
    ...
    if all_ready:
        ok("All datasets are already downloaded!")
        return
    ...
    for ds in DATASETS:
        s = dataset_status(ds)
        if s["exists"]:
            ok(f"{ds['name']}: already present ({s['size']})")
            continue

        if not args.all and not ds.get("auto"):
            resp = input(f"  Download {ds['name']}? [Y/n] ").strip().lower()
            if resp == "n":
                warn(f"Skipped {ds['name']}")
                continue

        info(f"Downloading {C.BOLD}{ds['name']}{C.RESET} ...")
        success = download_kaggle_dataset(ds["slug"], ds["expected_file"], ds["name"])
        if not success:
            fail(f"Failed to download {ds['name']}")
    ...
    if all_ready:
        print(f"\n{C.GREEN}{'='*60}{C.RESET}")
        print(f"{C.GREEN}✔ All datasets ready!{C.RESET}")
        print(f"{C.DIM}  Next step: python -m treatment_identifier.extract_data{C.RESET}")
        print(f"{C.GREEN}{'='*60}{C.RESET}\n")
    else:
        warn("Some datasets are still missing – check errors above")
